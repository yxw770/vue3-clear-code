...
export const state = reactive({
  ...
  battleListResolve: () => { }, // 對戰房間列表callback
  needBattleRoom: false, // 是否需要讀取對戰房間信息
  battleRoomUid: null, // 戰房間UID
  battleRoomResolve: () => { }, // 對戰房間信息callback
  battleRoomReject: () => { }, // 對戰房間信息callback
...
export function setupWebsocket(app) {
  ...
  window.wsPuseher.connector.pusher.connection.bind('disconnected', function () {
...
export function guestInit() {
  window.wsPuseher.channel("headerInfo").listen("HeaderInfoNotification", (data) => {
...
export function hasLoginInit() {
  ...
  // window.wsPuseher.connector.pusher.connection.bind('connecting', function () {
  ...
  //   console.error("disconnect")
  // })
  // window.wsPuseher.connector.pusher.connection.bind('connecting', () => {
  ...
  // window.wsPuseher.connector.pusher.connection.bind('unavailable', (payload) => {
  ...
  // window.wsPuseher.connector.pusher.connection.bind('failed', (payload) => {
  ...
  // window.wsPuseher.connector.pusher.connection.bind('disconnected', (payload) => {
  ...
  if (window.wsPuseher.options.authorizer) {
    ...
    window.wsPuseher.private("Player." + player_id).listen("PlayerNotificationEvent", (data) => {
...
export function getPlayerAmount(player_id) {
  ...
  window.wsPuseher.connector.pusher.send_event("getPlayerAmount", { player_id: player_id });
}
// 初始化讀取戰鬥房間列表
...
function battlesListNotify() {
  ...
  window.wsPuseher.channel("BattlesList").listen("BattlesListNotificationEvent", (e) => {
  ...
  window.wsPuseher.connector.pusher.send_event("battles", { join: { room: 'list', userId: 213123 } });
...
function battlesRoomNotify() {
  ...
  window.wsPuseher.channel("BattlesRoom." + state.battleRoomUid).listen("BattlesRoomNotificationEvent", (e) => {
  ...
  window.wsPuseher.connector.pusher.send_event("battles", { viewBattle: state.battleRoomUid });
...
function battlesRoomNotifyHandle(type, data, uid) {
  ...
  const types = {
    ...
    'battlePlaying': function () {
      ...
      window.dispatchEvent(new CustomEvent('battlePlaying-' + uid, {
        detail: data
    ...
    // battle完成
    'battleFinished': function () {
      window.dispatchEvent(new CustomEvent('battleFinished-' + uid, {
        detail: data
      }));
      // return battleRoomStore.setBaseData(data.data);
    },
